...
pub async fn seed_users_to_collection(db: &Database, collection_name: &str) -> SeedResult<usize> {
    ...
    if existing_count > 0 {
        println!("Collection '{}' already contains {} users. Skipping seed operation.", collection_name, existing_count);
        return Ok(0);
    }
    ...
    println!("Successfully seeded {} users into collection '{}'.", inserted_count, collection_name);
    Ok(inserted_count)
}
...
pub async fn clear_users_from_collection(db: &Database, collection_name: &str) -> SeedResult<u64> {
    ...
    println!("Successfully deleted {} users from collection '{}'.", deleted_count, collection_name);
    Ok(deleted_count)
}

/// Get the count of users in a specific collection
#[allow(dead_code)]
pub async fn get_user_count_from_collection(db: &Database, collection_name: &str) -> SeedResult<u64> {
    let collection: Collection<User> = db.collection(collection_name);
...
pub async fn reseed_users_to_collection(db: &Database, collection_name: &str) -> SeedResult<usize> {
    println!("Clearing existing users from collection '{}'...", collection_name);
...
mod tests {
    ...
    async fn setup_test_database() -> Option<Database> {
        ...
        for connection_string in connection_strings {
            match Client::with_uri_str(connection_string).await {
                Ok(client) => {
                    println!("Successfully connected to MongoDB for testing with: {}", connection_string);
    ...
    async fn test_seed_users_empty_database() {
        if let Some(db) = setup_test_database().await {
            ...
            assert!(count_result.is_ok(), "Failed to get user count: {:?}", count_result);
    ...
    async fn test_seed_users_non_empty_database() {
        if let Some(db) = setup_test_database().await {
            ...
            assert!(seed_result.is_ok(), "Failed to seed initial users: {:?}", seed_result);
            let initial_count = seed_result.unwrap();
            
            // Try to seed again - should return 0 since data already exists
            let result = seed_users_to_collection(&db, test_collection_name).await;
            assert!(result.is_ok(), "Failed to seed users second time: {:?}", result);
    ...
    async fn test_clear_users() {
        if let Some(db) = setup_test_database().await {
            ...
            let seed_result = seed_users_to_collection(&db, test_collection_name).await;
            assert!(seed_result.is_ok(), "Failed to seed users: {:?}", seed_result);
            ...
            let count_after_result = get_user_count_from_collection(&db, test_collection_name).await;
            assert!(count_after_result.is_ok(), "Failed to get user count after clear: {:?}", count_after_result);
    ...
    async fn test_clear_empty_database() {
        if let Some(db) = setup_test_database().await {
            ...
            // Try to clear again
            let result = clear_users_from_collection(&db, test_collection_name).await;
            assert!(result.is_ok(), "Failed to clear empty database: {:?}", result);
    ...
    async fn test_get_user_count() {
        if let Some(db) = setup_test_database().await {
            ...
            let empty_count_result = get_user_count_from_collection(&db, test_collection_name).await;
            assert!(empty_count_result.is_ok(), "Failed to get user count from empty database: {:?}", empty_count_result);
            ...
            let count_after_seed_result = get_user_count_from_collection(&db, test_collection_name).await;
            assert!(count_after_seed_result.is_ok(), "Failed to get user count after seed: {:?}", count_after_seed_result);
            ...
            assert!(clear_result2.is_ok(), "Failed to clear database second time: {:?}", clear_result2);
            
            let count_after_clear_result = get_user_count_from_collection(&db, test_collection_name).await;
            assert!(count_after_clear_result.is_ok(), "Failed to get user count after clear: {:?}", count_after_clear_result);
    ...
    async fn test_reseed_users() {
        if let Some(db) = setup_test_database().await {
            ...
            let initial_count_result = get_user_count_from_collection(&db, test_collection_name).await;
            assert!(initial_count_result.is_ok(), "Failed to get initial user count: {:?}", initial_count_result);
            ...
            assert!(insert_result.is_ok(), "Failed to insert additional user: {:?}", insert_result);
            
            let count_before_reseed_result = get_user_count_from_collection(&db, test_collection_name).await;
            assert!(count_before_reseed_result.is_ok(), "Failed to get user count before reseed: {:?}", count_before_reseed_result);
            ...
            let final_count_result = get_user_count_from_collection(&db, test_collection_name).await;
            assert!(final_count_result.is_ok(), "Failed to get final user count: {:?}", final_count_result);
